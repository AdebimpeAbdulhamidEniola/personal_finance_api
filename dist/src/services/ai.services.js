...
const formatTransactions = (transactions) => {
    return transactions.map(t => `- ${t.createdAt.toISOString().split('T')[0]}: ${t.description || 'No description'} | #${t.amount} | Category: ${t.category} | Type: ${t.type}`).join('\n');
};
/**
 * 1. GET INSIGHTS (Using Llama 3)
 */
export const generateInsights = async (transactions, monthlyIncome) => {
 ...
 */
export const generateBudgetPlan = async (transactions, monthlyIncome) => {
    const transactionSummary = formatTransactions(transactions);
    const completion = await client.chat.completions.create({
        messages: [
            {
